/// @description sWallDrawSetIndBigPipes()
...
vheight = 4//the minimum required height(in cells)
vwidth = 1//the minimum required width(in cells)
mingrounddist = 0//the minimum distance from the ground tile
maxgrounddist = 3//the maximum distance from the ground tile
minceildist = 0//the minimum distance from the "ceiling", the height of the bigwall.
maxceildist = 0//the maximum distance from the "ceiling"
bigwallchance = 20//the chance that a bigwall will be eligible
tilewallchance = 30//the chance that a tilewall will be eligible
pitchance = 2//the chance that a wall over a pit will be eligible
overobstacles = 1//if the dressing can be put behind obstacles or must be in cells above them
        ...
        if ds_list_size(ylist) > 0{
            ...
            switch southorwest{
                case 0://south facing wall
                    ...
                    dy = vtile.y+(vtile.elevation*elevheight)
                    //dy = vtile.y-((vup+1)*elevheight)
                    dy -= elevheight*(ay+1)
                    dy -= 3
                    //dy += irandom_range(-3,3)
                    tile_add(industrialbg1,bigwallw*7,bigwallh*irandom_range(2,4),bigwallw,bigwallh,dx,dy,vtile.depth+3)
                    ; break;
                case 1://west facing wall
                    ...
                    dy -= 3
                    //dy += irandom_range(-3,3)
                    tile_add(industrialbg1,bigwallw*6,bigwallh*irandom_range(2,4),bigwallw,bigwallh,dx,dy,vtile.depth+3)
                    ; break;
                }
