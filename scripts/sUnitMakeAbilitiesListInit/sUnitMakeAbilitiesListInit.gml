/// @description sUnitMakeAbilitiesListInit()
/*This script returns a list full of 300 zeroes. This is a unit's initial abilities list;
when an ability is checked for, this list will be checked to see if there is a 1 at the 
required index. Also used to make the cooldown lists and map use lists.*/
...
28 Taste for Combat, if the unit kills an enemy, it may make another attack without moving. There is no limit to how many
    attacks it may make this way.*******IMPLEMENTED**********
29 Sneaky, if the unit may move after attacking if it hasn't already moved.*******IMPLEMENTED*******
30 Wait for Me, the unit can never be left behind in a room if it has a clear path to the tram when it leaves.******IMPLEMENTED********
...
33 Booster, if this unit is next to a wall, friendly units can climb it to reach high places - up to 3 verts higher
    ...
    no additional penalty if this unit is on the tile at the bottom of the wall.*******IMPLEMENTED********
34 Lifter, friendly units gain 3 vertical distance with no additional penalty if this unit is on the tile at the
    top of the wall.*******IMPLEMENTED********
35 Sure Footing, the unit cannot be knocked this turn. Two turn cooldown.*****IMPLEMENTED********
36 Nimble, move through enemy units. Two turn cooldown.*******IMPLEMENTED********
37 Sprint, move twice as far for one turn. Two turn cooldown.*****IMPLEMENTED*******
...
42 First Aid, heal self or adjacent unit 10 HP without healing items. Three turn cooldown.*******IMPLEMENTED*******
43 First Aid 2, heal 20 hp***********IMPLEMENTED**********
44 First Aid 3, First Aid heals fully, but the cooldown is increased to four turns.*******IMPLEMENTED*********
45 Quick Study, if the unit would not learn an ability on level up, 30 percent chance to learn an ability.
46 Diversion, this unit gains a defensive bonus and enemy units are forced to target it next enemy turn. Three turn cooldown.
...
53 Reflexes, the next time this unit is attacked, that attack will miss. Includes counter attacks. Does not apply to area-of-effect damage. Two turn cooldown
